...
export default function ImportReceipts({ user }) {
  ...
  const handleConfirmSignature = () => {
    ...
    // 1. Thủ kho (Người lập) - required if saveAsDraft or fast-import (inspectingReceiptId === null)
    const needSig1 = saveAsDraft || inspectingReceiptId === null;
    // 2. Người giao hàng - required if saveAsDraft or fast-import (inspectingReceiptId === null)
    const needSig2 = saveAsDraft || inspectingReceiptId === null;
    // 3. Người kiểm 2 - required if completing inspection (!saveAsDraft)
  ...
  const handleSubmitImport = (e, saveAsDraft = false, sig1 = null, sig2 = null, sig3 = null) => {
    ...
    const hasRequiredSignatures = saveAsDraft 
      ? (sig1 && sig2)
      : (inspectingReceiptId !== null ? sig3 : (sig1 && sig2 && sig3));
                    ...
                    {[
                      ...
                      return (
                        <div key={dt.key} style={{ 
                          ...
                          <label style={{ 
                            cursor: inspectingReceiptId ? 'default' : 'pointer', 
                            display: 'flex', 
                            flexDirection: 'column',
                            alignItems: 'center', 
                            justifyContent: 'center',
                          ...
                          >
                            ...
                            <span style={{ fontSize: '0.72rem', fontWeight: '600', color: 'var(--text-muted)' }}>
                              {inspectingReceiptId ? 'Chỉ xem tệp đính kèm' : 'Nhấp để tải tệp lên'}
                            </span>
                            ...
                            <span 
                              onClick={(e) => {
                                ...
                                fetch(`/api/import/${imp.importID}`)
                                  ...
                                  .then(fullImp => setActiveReceiptForDossier(fullImp))
                                  .catch(err => alert("Lỗi: " + err.message));
                              }}
              ...
              {(() => {
                ...
                return (
                  <div style={{ marginTop: '2.5rem', borderTop: '2px solid #333', paddingTop: '1.2rem', pageBreakBefore: 'always' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                      {docs.map((doc, dIdx) => {
                        const isImage = doc.base64 && doc.base64.startsWith('data:image');
                        return (
              ...
              return (
                <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: '1.5rem', minHeight: '400px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', borderRight: '1px solid var(--border-glass)', paddingRight: '1rem', overflowY: 'auto', maxHeight: '480px' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 'bold', color: 'var(--text-dim)', marginBottom: '0.25rem', textTransform: 'uppercase' }}>Danh mục tài liệu</div>
                    {docs.map((doc, idx) => (
                      <button
                        key={idx}
                        className="btn-secondary"
                  ...
                  {/* Right panel: Active document viewer */}
                  {(() => {
                    const activeDoc = docs[activeDossierIndex] || docs[0];
                    if (!activeDoc) return null;
                    const isImage = activeDoc.base64 && activeDoc.base64.startsWith('data:image');
